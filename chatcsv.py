...
if API_KEY:
    ...
    if csv_file:
        ...
        # llm = AzureOpenAI(deployment_name=COMPLETION_MODEL ,openai_api_key= API_KEY , openai_api_version = API_VERSION)
        # agent = create_csv_agent(llm, "./TestFile2.csv", verbose=False)
...
# st.title("ChatCSV")
# st.write("Simple app to chat with a CSV file")
...
# def csv_agent(data):
#     llm = AzureOpenAI(deployment_name=COMPLETION_MODEL ,openai_api_key= API_KEY , openai_api_version = API_VERSION)
#     agent = create_csv_agent(llm, data , verbose=False)
#     return agent

# def query_agent(agent, query):
#     prompt = "You are a helpful AI Agent that helps processing \
#             CSV files using Langchain. Do not provide anything harmful\
#                 or incorrect. Be factual. Lets think step by step. Query is Query : "+ query
